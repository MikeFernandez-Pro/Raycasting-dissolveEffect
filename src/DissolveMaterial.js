...
export function DissolveMaterial({ baseMaterial, mode, thickness = 0.1, feather = 6, color = '#eb5a13', intensity = 50 }) {
  ...
  useFrame((state, delta) => {
    // objectRef.current.position.x = mouse.x
    // objectRef.current.position.y = mouse.y + 2
    raycaster.setFromCamera(mouse, camera)
    const intersects = raycaster.intersectObjects([objectRef.current, planeRef.current])
    if (intersects.length) {
      const point = intersects[0].point
      objectRef.current.position.copy(point.setZ(0.5))
    }
  })

  const uniforms = {
    uMatrix: { value: camera.matrixWorld },
    uFeather: { value: 6 },
  ...
  React.useEffect(() => {
    uniforms.uMatrix.value = objectRef.current.matrixWorld
  }, [uniforms.uMatrix.value])
  ...
  return (
    <>
      ...
      <mesh ref={objectRef} matrix={uniforms.uMatrix.value} scale={0.2} position-x={mouse.x} position-y={mouse.y + 2}>
        <sphereGeometry />
        <meshStandardMaterial visible={false} />
      </mesh>
      <mesh ref={planeRef} rotateX={Math.PI / 2} scale={10}>
        <planeGeometry />
